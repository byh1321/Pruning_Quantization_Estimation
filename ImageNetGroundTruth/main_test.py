...
parser.add_argument('--ne', default=5, type=int, help='number of epoch')
...
#    print('max : ',torch.max(output),', min : ',torch.min(output))
#    
#for m in net.modules():
...
#for m in net.modules():
#    if isinstance(m, nn.Conv2d):
#        print(torch.max(m.weight),torch.min(m.weight))
#    elif isinstance(m, nn.Linear):
#        print(torch.max(m.weight),torch.min(m.weight))
#test()
